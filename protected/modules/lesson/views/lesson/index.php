	...
	<div class="col-lg-12">
		...
		<div class="col-lg-6">								
			<p class="pull-right" style="display: block; margin-top: 4%;">
				<a href="<?php echo Yii::app()->controller->createUrl('/lesson/lesson/create'); ?>" class="btn btn-primary btn-lg">Додати</a>
			</p>									
		</div>
	</div>
</div>
				...
				'columns'=>array(
					...
					*/
					...
					array(
						...
						'value'=>'CHtml::link("Переглянути", array("/lesson/lesson/view", "id"=>$data->id), array("class"=>"btn btn-default"))',
						),
					...
					array(
						...
						'value'=>'CHtml::link("Редагувати", array("/lesson/update", "id"=>$data->id), array("class"=>"btn btn-default"))',
						),
					...
					array(
						...
						'value'=>'CHtml::link("Видалити", array("/lesson/delete", "id"=>$data->id), array("class"=>"btn btn-default", "confirm" => "Ви впевнені що хочете зробити цю операцію?"))',
			...
			<div class="col-lg-10 ">				
				<?php if ($pages>1): ?>
 				<ul class="pagination  pagination-lg pull-right" style="margin: 0">
					...
					<li <?php if ($p==$current) echo 'class="active"'; ?>><a href="<?php echo str_replace('module/', '', Yii::app()->createUrl('lesson/page/'.$p)); ?>"><?=$p?></a></li>
					<?php endfor; ?>
				</ul>
				<?php endif; ?>
